...
(function($) {
    ...
    function renderMap() {
      ...
      function createPins() {
        ...
        for (var i = 0; i < pins.length; i++) {
		  ...
		  var pinXPos = pins[i].xPos;
		  var pinYPos = pins[i].yPos;
		  
		  console.log('x circle =' + pinXPos);
		  console.log('y circle =' +pinYPos);
		  
          pin.data('id', i);
          pin.name =  pins[i].name;
		  pin.text = pins[i].text;
          ...
          pin.click(function(e) {
			...
			console.log('pins = '+isPin);
			
			
			console.log(id);
			console.log('mouse x' + mouseX);
			console.log('mouse y' + mouseY);
			//console.log(pins[i].xPos);
			//console.log(pins[i].yPos);
			
			pin.XPos = pins[id].xPos;
			pin.YPos = pins[id].yPos;
			
			console.log('posisi x =' + pin.XPos);
			console.log('posisi y =' + pin.YPos);
			
			
			//showTooltip(target.name);
			
			//isPin ? showTooltip(target.name) : removeTooltip();
			var xx = event.clientX;     // Get the horizontal coordinate
		    var yy = event.clientY;     // Get the vertical coordinate
		  
			modalPopUp(pins[id].text, xx, yy);
      ...
      function calculateTooltipOffset() {
		...
		console.log('Nilai y = '+ mouseY + tooltipOffsetY);
      }
  ...
  function modalPopUp(_text, _xPos, _yPos){
	  
	  console.log('x posisi = ' + _xPos);
	  console.log('y posisi= ' + _yPos);
		var txt;
		//var person = prompt("Please enter your name:", "Harry Potter");
		
		var name = 'DILo Bandung';
		var add1 = 'Alamat satu';
		var add2 = 'Alamat dua';
		...
		document.getElementById("myPopup").style.top = y.toString()+'px';
		document.getElementById("myPopup").style.left = x.toString()+'px';
		var str = _text;
		$("div").remove(".pin-modal");
		document.getElementById('myPopup').insertAdjacentHTML('beforeend',str);
		
		//document.getElementById("myPopup").style.position = "fixed:";
